...
void diagnose(const List<CXTranslationUnit> &units, uint32_t sourceFileId, IndexDataMessage &indexDataMessage, CreateLocation *locationInterface)
{
    for (auto tu : units) {
        std::function<void(CXDiagnostic, Diagnostics &, bool)> process = [&](CXDiagnostic d, Diagnostics &m, bool displayCategory) {
            ...
            if (type != Diagnostic::None) {
                ...
                const uint32_t fileId = locationInterface->createLocation(diagLoc, 0).fileId();
        ...
        for (unsigned int j=0; j<diagnosticCount; ++j) {
            ...
            if (inclusionError)
                indexDataMessage.setFlag(IndexDataMessage::InclusionError);
            assert(fileId);
            Flags<IndexDataMessage::FileFlag> &flags = indexDataMessage.files()[fileId];
            if (fileId != sourceFileId && !inclusionError && sev >= CXDiagnostic_Error && !(flags & IndexDataMessage::HeaderError)) {
            ...
            if (flags & IndexDataMessage::Visited) {
                process(diagnostic, indexDataMessage.diagnostics(), true);
            }
            ...
            for (unsigned int f=0; f<fixItCount; ++f) {
                ...
                const Location loc = locationInterface->createLocation(clang_getCString(fileName), line, column);
                if (indexDataMessage.files().value(loc.fileId()) & IndexDataMessage::Visited) {
                    unsigned int startOffset, endOffset;
                    ...
                    Diagnostic &entry = indexDataMessage.diagnostics()[Location(loc.fileId(), line, column)];
                    entry.type = Diagnostic::Fixit;
                    if (entry.message.isEmpty()) {
                        entry.message = String::format<64>("did you mean '%s'?", string);
                    }
                    entry.length = endOffset - startOffset;
                    indexDataMessage.fixIts()[loc.fileId()].insert(FixIt(line, column, endOffset - startOffset, string));
                }
            }
            clang_disposeDiagnostic(diagnostic);
        }

        for (const auto &it : indexDataMessage.files()) {
    ...
    for (const auto &it : indexDataMessage.files()) {
        if (it.second & IndexDataMessage::Visited) {
            const Location loc(it.first, 0, 0);
            const Map<Location, Diagnostic>::const_iterator x = indexDataMessage.diagnostics().lower_bound(loc);
            if (x == indexDataMessage.diagnostics().end() || x->first.fileId() != it.first) {
                indexDataMessage.diagnostics()[loc] = Diagnostic();
            }
        }
    }
}
